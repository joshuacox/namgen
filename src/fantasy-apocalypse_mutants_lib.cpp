 ...
 *
 * function generator$fantasy$apocalypse_mutants(type) {
 *   var namesFemale = [...];
 *   var namesMale   = [...];
 *   var namesNeutral = [...];
 *
...
std::string generate_fantasy_apocalypse_mutants_name(std::mt19937& rng, int type) {
    ...
    // Pick a random name from the selected list
    const std::string& name = (*selected)[rng() % selected->size()];
    return name;
}
